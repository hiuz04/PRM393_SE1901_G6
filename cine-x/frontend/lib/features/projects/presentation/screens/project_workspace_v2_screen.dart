            ...
            child: Theme(
              ...
              child: ExpansionTile(
                ...
                subtitle: act.description != null
                    ...
                    : null,
                trailing: _Badge(label: '${actScenes.length} cảnh'),
                children: [
          ...
          children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  ...scene.characters.map((character) {
                    final hasImage = !kIsWeb && character.imageUrl != null && character.imageUrl!.isNotEmpty;
                    return Tooltip(
                      message: '${character.name} (${characterRoleLabel(character.roleType)})',
                      child: Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            CircleAvatar(
                              ...
                              backgroundImage: hasImage ? FileImage(File(character.imageUrl!)) as ImageProvider : null,
                              child: !hasImage
                                  ? Text(
            ...
            if (scene.resources.isNotEmpty) ...[
              ...
              Text(
                ...
                style: const TextStyle(color: CineXPalette.textSecondary, fontSize: 12),
              ),
            ],
          ],
        ),
    ...
    return ListView(
      ...
      children: [
        ...
        if (provider.storyLocations.isEmpty)
          EmptyView(
            title: 'Chưa có bối cảnh truyện',
            message: 'Tạo một bối cảnh truyện (địa điểm trong kịch bản) để liên kết với các phân cảnh.',
            icon: Icons.theater_comedy_rounded,
            action: FilledButton.icon(
              onPressed: provider.can(ProjectPermission.manageStoryLocations)
                  ? () => _showStoryLocationSheet(context)
                  : null,
              icon: const Icon(Icons.add_location_alt_rounded),
              label: const Text('Thêm bối cảnh'),
            ),
          )
        else
            ...
            (location) => _Panel(
              child: ListTile(
                ...
                leading: Container(
                  ...
                  decoration: BoxDecoration(
                    color: (location.timeOfDay == 'NIGHT' ? Colors.indigo : Colors.orange).withAlpha(30),
                  ...
                  child: Icon(
                    ...
                    color: location.timeOfDay == 'NIGHT'
                        ? Colors.indigoAccent
                        : Colors.orange,
                    size: 24,
                  ),
                ...
                title: Row(
                  children: [
                    ...
                    const SizedBox(width: 6),
                    _Badge(label: location.settingType == 'EXT' ? 'EXT (Ngoại cảnh)' : 'INT (Nội cảnh)'),
                    const SizedBox(width: 4),
                    _Badge(label: location.timeOfDay == 'NIGHT' ? 'NIGHT (Đêm)' : 'DAY (Ngày)'),
                  ],
                ),
                subtitle: Padding(
    ...
    return ListView(
      ...
      children: [
        ...
        if (provider.shootingLocations.isEmpty)
          EmptyView(
            title: 'Chưa có địa điểm quay thực tế',
            message: 'Thêm địa điểm thực tế để hỗ trợ việc lập kế hoạch sản xuất.',
            icon: Icons.location_city_rounded,
            action: FilledButton.icon(
              onPressed: provider.can(ProjectPermission.manageShootingLocations)
                  ? () => _showShootingLocationSheet(context)
                  : null,
              icon: const Icon(Icons.add_location_alt_rounded),
              label: const Text('Thêm địa điểm quay'),
            ),
          )
        else
...
class _AnalyticsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final intScenes = provider.scenes.where((s) => s.settingType.toUpperCase() == 'INT').length;
    final extScenes = provider.scenes.where((s) => s.settingType.toUpperCase() == 'EXT').length;
      ...
      children: [
                  ...
                  PieChartData(
                    ...
                    sections: [
                      PieChartSectionData(
                        ...
                        title: 'Cần viết (${dashboard.todoScenes})',
                        color: CineXPalette.textSecondary,
                        radius: 50,
                      ),
                      PieChartSectionData(
                        value: dashboard.inProgressScenes.toDouble(),
                        title: 'Đang viết (${dashboard.inProgressScenes})',
                        color: CineXPalette.warning,
                        radius: 50,
                      ),
                      PieChartSectionData(
                        value: dashboard.doneScenes.toDouble(),
                        title: 'Xong (${dashboard.doneScenes})',
        ...
        _Panel(
          child: Column(
            ...
            children: [
              Text(
                'Tỷ lệ Nội cảnh (INT) vs Ngoại cảnh (EXT)',
                style: Theme.of(context).textTheme.titleMedium?.copyWith(
                      color: CineXPalette.textPrimary,
                      fontWeight: FontWeight.bold,
                    ),
              ...
              if (provider.scenes.isEmpty)
                const _MutedText('Chưa có cảnh nào để thống kê bối cảnh.')
              else
                SizedBox(
                  height: 200,
                  child: PieChart(
                    PieChartData(
                      ...
                      sections: [
                        PieChartSectionData(
                          value: intScenes.toDouble(),
                          title: 'INT - Nội cảnh ($intScenes)',
                          color: CineXPalette.primary,
                          radius: 50,
                        ),
                        PieChartSectionData(
                          value: extScenes.toDouble(),
                          title: 'EXT - Ngoại cảnh ($extScenes)',
...
class _StoryLocationSheetState extends State<_StoryLocationSheet> {
  ...
  void initState() {
    ...
    if (location != null) {
      ...
      _settingType = location.settingType;
      _timeOfDay = location.timeOfDay;
              ...
              children: [
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    decoration: const InputDecoration(labelText: 'Vị trí (Setting)'),
                    items: const [
                      DropdownMenuItem(value: 'INT', child: Text('INT (Trong nhà)')),
                      DropdownMenuItem(value: 'EXT', child: Text('EXT (Ngoài trời)')),
                    ],
                    onChanged: (value) => setState(() => _settingType = value ?? 'INT'),
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: const [
                      DropdownMenuItem(value: 'DAY', child: Text('DAY (Ban ngày)')),
                      DropdownMenuItem(value: 'NIGHT', child: Text('NIGHT (Ban đêm)')),
                    ],
                    onChanged: (value) => setState(() => _timeOfDay = value ?? 'DAY'),
                  ),
                ),
              ],
            ),
